...
ggChannelFilter::ggChannelFilter( const edm::ParameterSet& iConfig )
{
   ...
   _jetLimitPt = iConfig.getUntrackedParameter<double>( "jetLimit" , 25.0 );
...
void ggChannelFilter::endStream()
{
   ...
   std::cout << "Muon  Events: " << _muEventCount->GetBinContent(4) << endl; 
   std::cout << "Electron Events: " << _elEventCount->GetBinContent(4) << endl; 
}
...
bool ggChannelFilter::passJetCleaning( const edm::Event& iEvent , const edm::EventSetup& iSetup )
{
   ...
   for( const auto& jet : JetList ){
      if( isSelcJet( jet , _selcElecList, _selcMuonList, _selcJetCount , _jetLimitPt )){
